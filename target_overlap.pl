...
#target_overlap.pl TF_regul_targets_near.txt target_overlap_near.txt
#target_overlap.pl TF_regul_targets_near.txt TF_regul_targets_near_combined.txt -g
...
foreach my $TF (sort keys %group2compare){
	for(my $dir=0; $dir<2; $dir++){
		...
		foreach my $key (keys %hashIndirect){
			...
			if(@targets >=3 && $x1<2 || $x1>=100000){ next; }
